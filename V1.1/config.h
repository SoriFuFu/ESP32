...
private:
  ...
  bool loadFactoryDefaults()
  {
    ...
    if (!factoryFile)
    {
      Serial.println("Error al abrir el archivo de configuración de fábrica");
      return false;
    }
    ...
    if (error)
    {
      Serial.println("Error al analizar el archivo de configuración de fábrica");
      factoryFile.close();
      return false;
    }
...
public:
  ...
  void initSPIFFS() // MÉTODO PARA INICIAR EL SISTEMA DE ARCHIVOS SPIFFS
  {
    if (!SPIFFS.begin())
    {
      Serial.println("Fallo al montar el sistema de archivos SPIFFS");
      return;
    }
  }
