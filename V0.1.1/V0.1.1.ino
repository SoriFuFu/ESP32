...
ESP32Time rtc(3600);                                     // offset in seconds GMT+1                                        // OBJETO DEL RTC
...
void setMenuPage() // ESTABLECER LA PÁGINA DEL MENÚ EN LA PANTALLA
{
  ...
  else if (menuPage == 3)
  {
    ...
    displayLCD.page(K3Name, remainingTime[2]);
  }
  else if (menuPage == 4)
  {
    displayLCD.clearLine(1);
    ...
    displayLCD.page(K4Name, remainingTime[3]);
  }
}
...
void LCD_UpdateTimer(String relay) // ACTUALIZAR EL TEMPORIZADOR EN LA PANTALLA
{
  ...
  else if (relay == "K3" && menuPage == 3)
  {
    displayLCD.countPage(remainingTime[2]);
  }
  else if (relay == "K4" && menuPage == 4)
  {
    displayLCD.countPage(remainingTime[3]);
  }
}
...
void setManualTimer() // MOSTRAR PARPADEO
{
  ...
  else if (timer[2] == 1 && menuPage == 3) // MOSTRAR PARPADEO AL SELECCIONAR EL TIEMPO EN K3
  {
  ...
  else if (timer[3] == 1 && menuPage == 4) // MOSTRAR PARPADEO AL SELECCIONAR EL TIEMPO EN K4
  {
...
void initConnection(bool wifiActive, bool apActive)
{
  ...
  if (wifiActive && apActive) // Inicializar WiFi y AP
  {
    ...
    {
      Serial.println("Error al iniciar la conexión WiFi o el AP");
  ...
  else // Caso cuando ambos wifiActive y apActive son falsos
  {
    // Este caso no debería ocurrir, pero manejamos esto para asegurarnos de que siempre haya una conexión
...
void getMemory() // OBTENER LA MEMORIA LIBRE
{
  ...
  int freeHeapKb = freeHeap / 1024;
  displayLCD.printMemory(freeHeapKb);
}
